...
# Check columns exist
required_cols = ['Sensor Filename', 'Log Filename','Offset']
...
cases = list(zip(
    ...
    df_sel['Offset'].astype(str).str.strip()
))

print("\nSelected cases:")
for i, (s, l, off) in zip(rows, cases):
    print(f"Row {i}: {s} , {l} , Offset = {off}")


def process_case(sen_filename, log_filename,Offset):
...
for sen_file, log_file, Offset in cases:
    res = process_case(sen_file, log_file,Offset)
    results.append(res)
...
with PdfPages('test.pdf') as pdf:
    ...
    # ---- Y axis: auto from data ----
    all_alts = np.hstack([res['alt_as'] for res in results])

    alt_min_plot = max(0, np.nanmin(all_alts))
    alt_max_plot = np.nanmax(all_alts)

    # nice rounding
    alt_min_plot = 100 * np.floor(alt_min_plot / 100)
    alt_max_plot = 100 * np.ceil(alt_max_plot / 100)

    ax.set_ylim(alt_min_plot, alt_max_plot)
    ax.set_yticks(np.arange(alt_min_plot, alt_max_plot + 1, 500))
    ...
    for xv, res in zip(xvals, results):
        ...
        ax_in.set_xticks([0, 5, 10, 15,20,25])
